...
const ACTIVE_SESSION_STORAGE_KEY = "gemini-general-active-session-id";
...
export function GeminiChatClient() {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/conversations?counselorId=gemini");
      if (res.status === 401) {
        setNeedsAuth(true);
        return;
      }
  ...
  const handleShare = async () => {
    ...
    const text = messages
      .map((m) => `${m.role === "user" ? "あなた" : "ジェミニ"}: ${m.content}`)
      .join("\n\n");
  ...
  if (needsAuth) {
    return (
      <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-gradient-to-br from-[#fdf4ff] via-[#f3e5ff] to-[#fae8ff]">
        <div className="rounded-3xl bg-white px-10 py-12 text-center shadow-2xl">
          <p className="text-lg font-semibold text-[#9333ea]">ログインが必要です</p>
          <p className="mt-4 text-sm text-[#a855f7]">ジェミニAIをご利用いただくにはログインしてください。</p>
        </div>
      </div>
    );
  }
  ...
  const sessionSkeletonNodes = Array.from({ length: 4 }).map((_, idx) => (
    <div key={`session-skeleton-${idx}`} className="mb-2 h-12 animate-pulse rounded-2xl bg-[#f3e5ff]" />
  ));
  ...
  return (
    <div
      className="relative flex w-full flex-1 items-stretch bg-gradient-to-br from-[#fdf4ff] via-[#f3e5ff] to-[#fae8ff] text-[#7e22ce]"
      style={{
    ...
    >
      ...
      >
        <Button
          ...
          className="mb-6 w-full justify-center gap-2 rounded-2xl border border-black/15 bg-white text-[#9333ea] font-semibold shadow-sm transition hover:bg-[#fdf4ff]"
        >
          <Plus className="h-4 w-4" /> 新しいチャット
        </Button>
        <p className="mb-3 text-xs font-semibold uppercase tracking-[0.3em] text-[#a855f7]">チャット</p>
        <div className="flex-1 overflow-y-auto">
            ...
            sessions.map((session) => (
              ...
              >
                ...
                <button
                  ...
                  className="rounded-full p-1 text-[#c084fc] opacity-0 transition group-hover:opacity-100 hover:bg-white"
                >
          ...
          {sessions.length === 0 && !isLoading.sessions && (
            <p className="text-center text-xs text-[#d8b4fe]">まだチャット履歴がありません。</p>
          )}
        </div>
      </aside>
        ...
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative ml-auto flex h-full w-[80%] max-w-[300px] flex-col border-l border-[#fae8ff] bg-white/95 px-4 py-6 shadow-xl">
            ...
            <Button
              ...
              disabled={isLoading.sending}
              className="mb-4 w-full justify-center gap-2 rounded-2xl border border-black/15 bg-white text-[#9333ea] font-semibold shadow-sm transition hover:bg-[#fdf4ff]"
            >
              <Plus className="h-4 w-4" /> 新しいチャット
            </Button>
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.3em] text-[#a855f7]">チャット</p>
                ...
                sessions.map((session) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={(event) => handleDeleteSession(session.id, event)}
                      className="rounded-full p-1 text-[#c084fc] transition hover:bg-white"
                    >
                      <Trash2 className="h-3.5 w-3.5" />
                    </button>
                  </div>
      ...
      <main className="flex h-full min-w-0 flex-1 flex-col overflow-hidden bg-white/75 touch-auto overscroll-none">
        <header className="flex items-center justify-between border-b border-[#fae8ff] px-4 py-3 text-sm text-[#a855f7]">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-[#9333ea]">{activeSession?.title || "ジェミニ 二つの視点で整えるAI"}</span>
            {isLoading.messages && messages.length === 0 && <Loader2 className="h-4 w-4 animate-spin text-[#c084fc]" />}
          </div>
          {messages.length > 0 && (
            <Button variant="ghost" size="sm" className="text-[#a855f7]" onClick={handleShare}>
              <Share2 className="mr-2 h-4 w-4" /> 共有
            </Button>
          )}
        </header>

        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto bg-gradient-to-b from-white via-[#fdf4ff] to-[#f3e5ff]" style={{ WebkitOverflowScrolling: "touch" }}>
              ...
              <div className="flex h-full flex-col items-center justify-center gap-8">
                <div className="flex flex-col items-center gap-4">
                  <div className="relative h-32 w-32 overflow-hidden rounded-2xl bg-white shadow-lg">
                    <Image src="/images/counselors/gemini.png" alt="Gemini" fill className="object-contain" sizes="128px" />
                  </div>
                  <div className="text-center">
                    <h2 className="text-2xl font-bold text-[#9333ea]">こんにちは、ジェミニです</h2>
                    <p className="mt-2 text-sm text-[#a855f7]">物事を多角的に捉えて整理します。</p>
                    <p className="mt-1 text-sm text-[#a855f7]">二つの面から考えることで、新しい気づきを与えます。</p>
                  </div>
                </div>
                    ...
                    <div className={cn("flex gap-3", message.role === "user" ? "justify-end" : "justify-start")}>
                      {message.role === "assistant" && (
                        <div className="relative h-12 w-12 flex-shrink-0 overflow-hidden rounded-xl bg-white shadow">
                          <Image src="/images/counselors/gemini.png" alt="Gemini" fill className="object-contain" sizes="48px" />
                        </div>
                      )}
                      <div
                        className={cn(
                        ...
                        {message.pending && (
                          <div className="flex items-center gap-2">
                            <div className="flex gap-1">
                              <div className="h-2 w-2 animate-bounce rounded-full bg-[#a855f7] [animation-delay:-0.3s]" />
                              <div className="h-2 w-2 animate-bounce rounded-full bg-[#a855f7] [animation-delay:-0.15s]" />
                              <div className="h-2 w-2 animate-bounce rounded-full bg-[#a855f7]" />
                            </div>
                            <p className="text-xs text-[#c084fc]">{thinkingMessages[currentThinkingIndex]}</p>
                          </div>
                        )}
                      </div>
                      {message.role === "user" && (
                        <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-[#9333ea] shadow">
        ...
        <div
          ref={composerRef}
          className="sticky bottom-0 left-0 right-0 border-t border-[#fae8ff] bg-white/95 px-4 pt-2 z-50"
          style={{
            paddingBottom: "calc(env(safe-area-inset-bottom) + 0.5rem)",
          }}
        >
          <div className="mx-auto max-w-3xl">
            {error && <p className="mb-2 text-xs font-medium text-[#9333ea]">{error}</p>}
            <div className="flex items-end gap-3 rounded-3xl border border-[#fae8ff] bg-white/90 px-4 py-3 shadow-sm">
              <textarea
                ref={textareaRef}
                value={input}
                ...
                placeholder="ジェミニに話しかける..."
                enterKeyHint="send"
                autoComplete="off"
                autoCorrect="off"
                autoCapitalize="off"
                disabled={isLoading.sending}
                className="max-h-40 flex-1 resize-none border-0 bg-transparent px-1 py-2 text-base leading-relaxed text-[#7e22ce] placeholder:text-[#d8b4fe] focus:outline-none disabled:opacity-60 md:text-sm"
                rows={1}
              />
              <button
                type="button"
                ...
                className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-[#a855f7] to-[#c084fc] text-white shadow-lg transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isLoading.sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
              </button>
            </div>
            <p className="mt-2 text-center text-xs text-[#c084fc]">ジェミニAIは誤った情報を生成する場合があります。</p>
          </div>
        </div>
      </main>
    </div>

...
const ACTIVE_SESSION_STORAGE_KEY = "nazare-bible-active-session-id";
...
export function NazareChatClient() {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/conversations?counselorId=nazare");
      if (res.status === 401) {
        setNeedsAuth(true);
        return;
      }
  ...
  const handleShare = async () => {
    ...
    const text = messages
      .map((m) => `${m.role === "user" ? "あなた" : "ナザレ"}: ${m.content}`)
      .join("\n\n");
  ...
  if (needsAuth) {
    return (
      <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-gradient-to-br from-[#faf5ff] via-[#f3e8ff] to-[#e9d5ff]">
        <div className="rounded-3xl bg-white px-10 py-12 text-center shadow-2xl">
          <p className="text-lg font-semibold text-[#7c3aed]">ログインが必要です</p>
          <p className="mt-4 text-sm text-[#8b5cf6]">ナザレAIをご利用いただくにはログインしてください。</p>
        </div>
      </div>
    );
  }
  ...
  const sessionSkeletonNodes = Array.from({ length: 4 }).map((_, idx) => (
    <div key={`session-skeleton-${idx}`} className="mb-2 h-12 animate-pulse rounded-2xl bg-[#f3e8ff]" />
  ));
    ...
    <div className="mx-auto max-w-3xl space-y-4" style={{ paddingBottom: `${messagePaddingBottom}px` }}>
      {Array.from({ length: 3 }).map((_, idx) => (
        <div key={`message-skeleton-${idx}`} className="flex gap-3">
          <div className="h-10 w-10 rounded-full bg-[#ddd6fe]/60" />
  ...
  return (
    ...
    >
      ...
      <aside
        className="hidden w-[260px] min-w-[260px] flex-col border-r border-[#ddd6fe] bg-white/90 px-4 py-6 shadow-sm md:flex md:sticky md:top-16 md:self-start md:overflow-y-auto"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
              ...
              <div
                ...
                className={cn(
                  ...
                  session.id === activeSessionId
                    ? "border-[#c4b5fd] bg-[#f5f3ff] text-[#6b21a8]"
                    : "border-transparent bg-transparent text-[#8b5cf6] hover:border-[#ddd6fe] hover:bg-[#faf5ff]",
                )}
              >
          ...
          {sessions.length === 0 && !isLoading.sessions && (
            <p className="text-center text-xs text-[#c4b5fd]">まだチャット履歴がありません。</p>
          )}
        </div>
      </aside>
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative ml-auto flex h-full w-[80%] max-w-[300px] flex-col border-l border-[#ddd6fe] bg-white/95 px-4 py-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <span className="text-sm font-semibold text-[#7c3aed]">履歴</span>
      ...
      <main className="flex h-full min-w-0 flex-1 flex-col overflow-hidden bg-white/75 touch-auto overscroll-none">
        <header className="flex items-center justify-between border-b border-[#e9d5ff] px-4 py-3 text-sm text-[#8b5cf6]">
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="icon"
              className="rounded-full border border-[#ddd6fe] bg-white text-[#8b5cf6] hover:bg-[#faf5ff] md:hidden"
              onClick={() => setIsSidebarOpen(true)}
            >
              <Menu className="h-5 w-5" />
            </Button>
            <span className="font-semibold text-[#7c3aed]">{activeSession?.title || "ナザレ 聖書カウンセリング"}</span>
            {isLoading.messages && messages.length === 0 && <Loader2 className="h-4 w-4 animate-spin text-[#a78bfa]" />}
          </div>
          {messages.length > 0 && (
            <Button variant="ghost" size="sm" className="text-[#8b5cf6]" onClick={handleShare}>
              <Share2 className="mr-2 h-4 w-4" /> 共有
            </Button>
          )}
        </header>

        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto bg-gradient-to-b from-white via-[#faf5ff] to-[#f3e8ff]" style={{ WebkitOverflowScrolling: "touch" }}>
              ...
              <div className="flex h-full flex-col items-center justify-center gap-8">
                <div className="flex flex-col items-center gap-4">
                  <div className="relative h-32 w-32 overflow-hidden rounded-2xl bg-white shadow-lg">
                    <Image src="/images/counselors/nazare.png" alt="Nazare" fill className="object-contain" sizes="128px" />
                  </div>
                  <div className="text-center">
                    <h2 className="text-2xl font-bold text-[#7c3aed]">こんにちは、ナザレです</h2>
                    <p className="mt-2 text-sm text-[#8b5cf6]">心の重荷を、御言葉と共に降ろしませんか。</p>
                    <p className="mt-1 text-sm text-[#8b5cf6]">静かに耳を傾け、祈りながらお話を伺います。</p>
                  </div>
                </div>
                    ...
                    <div className={cn("flex gap-3", message.role === "user" ? "justify-end" : "justify-start")}>
                      {message.role === "assistant" && (
                        <div className="relative h-12 w-12 flex-shrink-0 overflow-hidden rounded-xl bg-white shadow">
                          <Image src="/images/counselors/nazare.png" alt="Nazare" fill className="object-contain" sizes="48px" />
                        </div>
                      )}
                      <div
                        className={cn(
                          ...
                          message.role === "user"
                            ...
                            : "bg-white border border-[#ddd6fe] text-[#5b21b6]",
                        )}
                      >
                        ...
                        {message.pending && (
                          <div className="flex items-center gap-2">
                            <div className="flex gap-1">
                              <div className="h-2 w-2 animate-bounce rounded-full bg-[#8b5cf6] [animation-delay:-0.3s]" />
                              <div className="h-2 w-2 animate-bounce rounded-full bg-[#8b5cf6] [animation-delay:-0.15s]" />
                              <div className="h-2 w-2 animate-bounce rounded-full bg-[#8b5cf6]" />
                            </div>
                            <p className="text-xs text-[#a78bfa]">{thinkingMessages[currentThinkingIndex]}</p>
                          </div>
                        )}
                      </div>
                      {message.role === "user" && (
                        <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-[#7c3aed] shadow">
        ...
        <div
          ref={composerRef}
          className="sticky bottom-0 left-0 right-0 border-t border-[#e9d5ff] bg-white/95 px-4 pt-2 z-50"
          style={{
            paddingBottom: "calc(env(safe-area-inset-bottom) + 0.5rem)",
          }}
        >
          <div className="mx-auto max-w-3xl">
            {error && <p className="mb-2 text-xs font-medium text-[#7c3aed]">{error}</p>}
            <div className="flex items-end gap-3 rounded-3xl border border-[#e9d5ff] bg-white/90 px-4 py-3 shadow-sm">
              <textarea
                ref={textareaRef}
                value={input}
...
export default NazareChatClient;

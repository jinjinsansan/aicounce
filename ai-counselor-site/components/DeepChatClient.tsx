...
const ACTIVE_SESSION_STORAGE_KEY = "deep-analysis-active-session-id";
...
export function DeepChatClient() {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/conversations?counselorId=deep");
      if (res.status === 401) {
        setNeedsAuth(true);
        return;
      }
  ...
  const handleShare = async () => {
    ...
    const text = messages
      .map((m) => `${m.role === "user" ? "あなた" : "ディープ"}: ${m.content}`)
      .join("\n\n");
  ...
  if (needsAuth) {
    return (
      <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-gradient-to-br from-[#ecfeff] via-[#cffafe] to-[#ccfbf1]">
        <div className="rounded-3xl bg-white px-10 py-12 text-center shadow-2xl">
          <p className="text-lg font-semibold text-[#0d9488]">ログインが必要です</p>
          <p className="mt-4 text-sm text-[#14b8a6]">ディープAIをご利用いただくにはログインしてください。</p>
        </div>
      </div>
    );
  }
  ...
  const sessionSkeletonNodes = Array.from({ length: 4 }).map((_, idx) => (
    <div key={`session-skeleton-${idx}`} className="mb-2 h-12 animate-pulse rounded-2xl bg-[#cffafe]" />
  ));
    ...
    <div className="mx-auto max-w-3xl space-y-4" style={{ paddingBottom: `${messagePaddingBottom}px` }}>
      {Array.from({ length: 3 }).map((_, idx) => (
        <div key={`message-skeleton-${idx}`} className="flex gap-3">
          <div className="h-10 w-10 rounded-full bg-[#99f6e4]/60" />
  ...
  return (
    <div
      className="relative flex w-full flex-1 items-stretch bg-gradient-to-br from-[#ecfeff] via-[#cffafe] to-[#ccfbf1] text-[#115e59]"
      style={{
    ...
    >
      ...
      <aside
        className="hidden w-[260px] min-w-[260px] flex-col border-r border-[#99f6e4] bg-white/90 px-4 py-6 shadow-sm md:flex md:sticky md:top-16 md:self-start md:overflow-y-auto"
      ...
      >
        <Button
          ...
          className="mb-6 w-full justify-center gap-2 rounded-2xl border border-black/15 bg-white text-[#0d9488] font-semibold shadow-sm transition hover:bg-[#ecfeff]"
        >
        ...
        <div className="flex-1 overflow-y-auto">
              ...
              <div
                ...
                className={cn(
                  ...
                  session.id === activeSessionId
                    ? "border-[#2dd4bf] bg-[#f5f3ff] text-[#134e4a]"
                    : "border-transparent bg-transparent text-[#14b8a6] hover:border-[#99f6e4] hover:bg-[#ecfeff]",
                )}
              >
                ...
                <button
                  ...
                  className="rounded-full p-1 text-[#2dd4bf] opacity-0 transition group-hover:opacity-100 hover:bg-white"
                >
          ...
          {sessions.length === 0 && !isLoading.sessions && (
            <p className="text-center text-xs text-[#2dd4bf]">まだチャット履歴がありません。</p>
          )}
        </div>
      </aside>
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative ml-auto flex h-full w-[80%] max-w-[300px] flex-col border-l border-[#99f6e4] bg-white/95 px-4 py-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <span className="text-sm font-semibold text-[#0d9488]">履歴</span>
            ...
            <Button
              ...
              disabled={isLoading.sending}
              className="mb-4 w-full justify-center gap-2 rounded-2xl border border-black/15 bg-white text-[#0d9488] font-semibold shadow-sm transition hover:bg-[#ecfeff]"
            >
              <Plus className="h-4 w-4" /> 新しいチャット
            </Button>
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.3em] text-[#14b8a6]">チャット</p>
                ...
                sessions.map((session) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={(event) => handleDeleteSession(session.id, event)}
                      className="rounded-full p-1 text-[#2dd4bf] transition hover:bg-white"
                    >
                      <Trash2 className="h-3.5 w-3.5" />
                    </button>
                  </div>
      ...
      <main className="flex h-full min-w-0 flex-1 flex-col overflow-hidden bg-white/75 touch-auto overscroll-none">
        <header className="flex items-center justify-between border-b border-[#ccfbf1] px-4 py-3 text-sm text-[#14b8a6]">
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="icon"
              className="rounded-full border border-[#99f6e4] bg-white text-[#14b8a6] hover:bg-[#ecfeff] md:hidden"
              onClick={() => setIsSidebarOpen(true)}
            >
              <Menu className="h-5 w-5" />
            </Button>
            <span className="font-semibold text-[#0d9488]">{activeSession?.title || "ディープ 分析型AIカウンセラー"}</span>
            {isLoading.messages && messages.length === 0 && <Loader2 className="h-4 w-4 animate-spin text-[#2dd4bf]" />}
          </div>
          {messages.length > 0 && (
            <Button variant="ghost" size="sm" className="text-[#14b8a6]" onClick={handleShare}>
              <Share2 className="mr-2 h-4 w-4" /> 共有
            </Button>
          )}
        </header>

        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto bg-gradient-to-b from-white via-[#ecfeff] to-[#cffafe]" style={{ WebkitOverflowScrolling: "touch" }}>
              ...
              <div className="flex h-full flex-col items-center justify-center gap-8">
                <div className="flex flex-col items-center gap-4">
                  <div className="relative h-32 w-32 overflow-hidden rounded-2xl bg-white shadow-lg">
                    <Image src="/images/counselors/deep.png" alt="Deep" fill className="object-contain" sizes="128px" />
                  </div>
                  <div className="text-center">
                    <h2 className="text-2xl font-bold text-[#0d9488]">こんにちは、ディープです</h2>
                    <p className="mt-2 text-sm text-[#14b8a6]">要因を分解しながら提案します。</p>
                    <p className="mt-1 text-sm text-[#14b8a6]">詳細に探求し、複雑な問いに向き合います。</p>
                  </div>
                </div>
                ...
                <div className="grid w-full max-w-xl grid-cols-2 gap-3 px-4 md:grid-cols-4">
                  {initialPrompts.map((prompt) => (
                    <button
                      ...
                      disabled={isLoading.sending}
                      className="rounded-xl border border-[#99f6e4] bg-white px-6 py-4 text-center text-sm text-[#115e59] shadow-sm transition-all hover:bg-[#ecfeff] hover:shadow-md disabled:opacity-50"
                    >
                      {prompt}
                    </button>
                  ))}
                    ...
                    <div className={cn("flex gap-3", message.role === "user" ? "justify-end" : "justify-start")}>
                      {message.role === "assistant" && (
                        <div className="relative h-12 w-12 flex-shrink-0 overflow-hidden rounded-xl bg-white shadow">
                          <Image src="/images/counselors/deep.png" alt="Deep" fill className="object-contain" sizes="48px" />
                        </div>
                      )}
                      <div
                        className={cn(
                          ...
                          message.role === "user"
                            ...
                            : "bg-white border border-[#99f6e4] text-[#115e59]",
                        )}
                      >
                        ...
                        {message.pending && (
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-xs text-[#2dd4bf]">{thinkingMessages[currentThinkingIndex]}</p>
                          </div>
                        )}
                      </div>
                      {message.role === "user" && (
                        <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-[#0d9488] shadow">
        ...
        <div
          ref={composerRef}
          className="sticky bottom-0 left-0 right-0 border-t border-[#ccfbf1] bg-white/95 px-4 pt-2 z-50"
          style={{
            paddingBottom: "calc(env(safe-area-inset-bottom) + 0.5rem)",
          }}
        >
          <div className="mx-auto max-w-3xl">
            {error && <p className="mb-2 text-xs font-medium text-[#0d9488]">{error}</p>}
            <div className="flex items-end gap-3 rounded-3xl border border-[#ccfbf1] bg-white/90 px-4 py-3 shadow-sm">
              <textarea
                ref={textareaRef}
                value={input}
                ...
                placeholder="ディープに話しかける..."
                enterKeyHint="send"
                autoComplete="off"
                autoCorrect="off"
                autoCapitalize="off"
                disabled={isLoading.sending}
                className="max-h-40 flex-1 resize-none border-0 bg-transparent px-1 py-2 text-base leading-relaxed text-[#115e59] placeholder:text-[#2dd4bf] focus:outline-none disabled:opacity-60 md:text-sm"
                rows={1}
              />
              <button
                type="button"
                ...
                className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-[#14b8a6] to-[#2dd4bf] text-white shadow-lg transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isLoading.sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
              </button>
            </div>
            <p className="mt-2 text-center text-xs text-[#2dd4bf]">ディープAIは誤った情報を生成する場合があります。</p>
          </div>
        </div>
      </main>
    </div>

...
const ACTIVE_SESSION_STORAGE_KEY = "adam-general-active-session-id";
...
export function AdamChatClient() {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/conversations?counselorId=adam");
      if (res.status === 401) {
        setNeedsAuth(true);
        return;
      }
  ...
  const handleShare = async () => {
    ...
    const text = messages
      .map((m) => `${m.role === "user" ? "あなた" : "アダム"}: ${m.content}`)
      .join("\n\n");
  ...
  if (needsAuth) {
    return (
      <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-gradient-to-br from-[#f0fdfa] via-[#ccfbf1] to-[#d1fae5]">
        <div className="rounded-3xl bg-white px-10 py-12 text-center shadow-2xl">
          ...
          <p className="mt-4 text-sm text-[#14b8a6]">アダムAIをご利用いただくにはログインしてください。</p>
        </div>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-3xl space-y-4" style={{ paddingBottom: `${messagePaddingBottom}px` }}>
      {Array.from({ length: 3 }).map((_, idx) => (
        <div key={`message-skeleton-${idx}`} className="flex gap-3">
          <div className="h-10 w-10 rounded-full bg-[#ccfbf1]/60" />
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
              ...
              <div
                ...
                className={cn(
                  ...
                  session.id === activeSessionId
                    ? "border-[#5eead4] bg-[#f0fdfa] text-[#115e59]"
                    : "border-transparent bg-transparent text-[#14b8a6] hover:border-[#99f6e4] hover:bg-[#f0fdfa]",
                )}
              >
          ...
          {sessions.length === 0 && !isLoading.sessions && (
            <p className="text-center text-xs text-[#99f6e4]">まだチャット履歴がありません。</p>
          )}
        </div>
      </aside>
        ...
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative ml-auto flex h-full w-[80%] max-w-[300px] flex-col border-l border-[#99f6e4] bg-white/95 px-4 py-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <span className="text-sm font-semibold text-[#0f766e]">履歴</span>
      ...
      <main className="flex h-full min-w-0 flex-1 flex-col overflow-hidden bg-white/75 touch-auto overscroll-none">
        <header className="flex items-center justify-between border-b border-[#a7f3d0] px-4 py-3 text-sm text-[#14b8a6]">
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="icon"
              className="rounded-full border border-[#99f6e4] bg-white text-[#14b8a6] hover:bg-[#f0fdfa] md:hidden"
              onClick={() => setIsSidebarOpen(true)}
            >
              <Menu className="h-5 w-5" />
            </Button>
            <span className="font-semibold text-[#0f766e]">{activeSession?.title || "アダム 万能型カウンセラー"}</span>
            {isLoading.messages && messages.length === 0 && <Loader2 className="h-4 w-4 animate-spin text-[#5eead4]" />}
          </div>
          {messages.length > 0 && (
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto bg-gradient-to-b from-white via-[#f0fdfa] to-[#ccfbf1]" style={{ WebkitOverflowScrolling: "touch" }}>
              ...
              <div className="flex h-full flex-col items-center justify-center gap-8">
                <div className="flex flex-col items-center gap-4">
                  <div className="relative h-32 w-32 overflow-hidden rounded-2xl bg-white shadow-lg">
                    <Image src="/images/counselors/adam.png" alt="Adam" fill className="object-contain" sizes="128px" />
                  </div>
                  <div className="text-center">
                    <h2 className="text-2xl font-bold text-[#0f766e]">こんにちは、アダムです</h2>
                    <p className="mt-2 text-sm text-[#14b8a6]">どんな悩みでも、中立的な視点でお聞きします。</p>
                    <p className="mt-1 text-sm text-[#14b8a6]">実用的で、バランスの取れたアドバイスを提供します。</p>
                  </div>
                </div>
                    ...
                    <div className={cn("flex gap-3", message.role === "user" ? "justify-end" : "justify-start")}>
                      {message.role === "assistant" && (
                        <div className="relative h-12 w-12 flex-shrink-0 overflow-hidden rounded-xl bg-white shadow">
                          <Image src="/images/counselors/adam.png" alt="Adam" fill className="object-contain" sizes="48px" />
                        </div>
                      )}
                      <div
                        className={cn(
                          ...
                          message.role === "user"
                            ...
                            : "bg-white border border-[#99f6e4] text-[#0f766e]",
                        )}
                      >
                        ...
                        {message.pending && (
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-xs text-[#5eead4]">{thinkingMessages[currentThinkingIndex]}</p>
                          </div>
                        )}
                      </div>
                      {message.role === "user" && (
                        <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-[#0f766e] shadow">
        ...
        <div
          ref={composerRef}
          className="sticky bottom-0 left-0 right-0 border-t border-[#a7f3d0] bg-white/95 px-4 pt-2 z-50"
          style={{
            paddingBottom: "calc(env(safe-area-inset-bottom) + 0.5rem)",
          }}
        >
          <div className="mx-auto max-w-3xl">
            {error && <p className="mb-2 text-xs font-medium text-[#0f766e]">{error}</p>}
            <div className="flex items-end gap-3 rounded-3xl border border-[#a7f3d0] bg-white/90 px-4 py-3 shadow-sm">
              <textarea
                ref={textareaRef}
                value={input}

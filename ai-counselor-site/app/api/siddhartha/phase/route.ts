...
export async function POST(request: Request) {
  ...
  const { data: messages, error: messageError } = await supabase
    .from("siddhartha_messages" as any)
  ...
  if (messageError) {
    console.error("Failed to load siddhartha messages for phase insight", messageError);
  ...
  const conversation = buildConversationSnapshot(messages as any, 18);
...
出力形式:
{
  ...
  try {
    const completion = await openai.chat.completions.create({
      model: siddharthaEnv.phaseModel,
      temperature: 0.2,

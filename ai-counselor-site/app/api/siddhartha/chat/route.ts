...
import { getSiddharthaAssistantId, getSiddharthaOpenAIClient } from "@/lib/siddhartha/openai";
import { retrieveKnowledgeMatches } from "@/lib/siddhartha/rag";
...
type OpenAIThreads = NonNullable<NonNullable<ReturnType<typeof getSiddharthaOpenAIClient>["beta"]>["threads"]>;
...
export async function POST(request: Request) {
  ...
  if (hasServiceRole()) {
    try {
      ...
      console.error("[Siddhartha Chat] Failed to upsert user profile", error);
    }
  }
  ...
  try {
    ...
    // RAG検索
    console.log(`[Siddhartha Chat] User message: "${parsed.data.message.slice(0, 50)}..."`);
    const knowledgeMatches = await retrieveKnowledgeMatches(parsed.data.message, {
      matchCount: 8,
      similarityThreshold: 0.45,
    });

    console.log(`[Siddhartha Chat] RAG matches: ${knowledgeMatches.length}`);
    if (knowledgeMatches.length > 0) {
      console.log(
        `[Siddhartha Chat] Top 3 similarities:`,
        knowledgeMatches
          .slice(0, 3)
    ...
    // ユーザーメッセージ保存
    await supabase.from("siddhartha_messages" as any).insert({
    ...
    const openai = getSiddharthaOpenAIClient();
    ...
    try {
      ...
      console.error("[Siddhartha Chat] OpenAI message creation error:", openaiError);
    ...
    console.log("[Siddhartha Chat] Starting assistant completion...");
    const assistantResponse = await runBufferedCompletion(betaThreads, threadId);
    console.log(`[Siddhartha Chat] Assistant response length: ${assistantResponse.length} chars`);
    ...
    console.log("[Siddhartha Chat] Chat completion successful");
    return NextResponse.json({
      sessionId,
    ...
    console.error("Siddhartha chat error", error);
...
const resolveSession = async (
  ...
  if (sessionId) {
    ...
    const threadId = await ensureThreadId(supabase, (data as any).id, (data as any).openai_thread_id);
    return { sessionId: (data as any).id, threadId };
  }
  ...
  const threadId = await ensureThreadId(supabase, (data as any).id, null);
  return { sessionId: (data as any).id, threadId };
};
...
const ensureThreadId = async (
  ...
  await supabase.from("siddhartha_sessions" as any).update({ openai_thread_id: thread.id }).eq("id", sessionId);
  return thread.id;
};

const runBufferedCompletion = async (threads: OpenAIThreads, threadId: string) => {
  const assistantId = getSiddharthaAssistantId();
  let fullReply = "";
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    stream
      ...
      .on("error", (error: unknown) => {
        console.error("[Siddhartha Chat] Stream error:", error);
        reject(error);
      })
      .on("end", () => {
        resolve();

...
const PARENT_DIR = path.join(ROOT_DIR, "data/dale-rag/chunks/parent");
const CHILD_DIR = path.join(ROOT_DIR, "data/dale-rag/chunks/child");
...
const parseChunk = async (filePath: string): Promise<Chunk> => {
  ...
  const sourceType = "dale_carnegie";
...
async function clearExistingRagData() {
  ...
  const { error } = await supabase.from("rag_documents").delete().eq("counselor_id", "dale");
  if (error) {
    throw error;
  }
  console.log("🧹 既存のデール向けRAGドキュメントを削除しました");
}
...
async function main() {
  ...
  console.log("✅ デールのRAGデータをSupabaseに登録しました");
}

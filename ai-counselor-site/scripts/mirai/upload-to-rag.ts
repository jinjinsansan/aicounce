...
const PARENT_DIR = path.join(ROOT_DIR, "data/mirai-rag/chunks/parent");
const CHILD_DIR = path.join(ROOT_DIR, "data/mirai-rag/chunks/child");
...
const parseChunk = async (filePath: string): Promise<Chunk> => {
  ...
  const sourceType = typeof metadata.source === "string" ? metadata.source : "mirai_rag";
...
async function clearExistingRagData() {
  ...
  const { error } = await supabase.from("rag_documents").delete().eq("counselor_id", "mirai");
  if (error) {
    throw error;
  }
  console.log("🧹 既存のミライ向けRAGドキュメントを削除しました");
}
...
async function main() {
  ...
  console.log("✅ ミライのRAGデータをSupabaseに登録しました");
}

...
const SOURCE_DIR = path.join(ROOT_DIR, "data/nana-rag/chunks/parent");
...
const parseParentChunk = async (filePath: string): Promise<ParentChunk> => {
  ...
  const sourceType = typeof metadata.source === "string" ? metadata.source : "mhsw_rag_system";
...
async function loadParentChunks(): Promise<ParentChunk[]> {
  const files = await fs.readdir(SOURCE_DIR);
  const markdownFiles = files.filter((file) => file.endsWith(".md"));
  markdownFiles.sort();
  const chunks: ParentChunk[] = [];
  for (const file of markdownFiles) {
    const parsed = await parseParentChunk(path.join(SOURCE_DIR, file));
    chunks.push(parsed);
  }
  return chunks;
}
...
async function clearExistingRagData() {
  ...
  const { error } = await supabase.from("rag_documents").delete().eq("counselor_id", "nana");
  if (error) {
    throw error;
  }
  console.log("🧹 既存のナナ向けRAGドキュメントを削除しました");
}

async function main() {
  const shouldClear = process.argv.includes("--clear") || process.argv.includes("-c");

  const parentChunks = await loadParentChunks();
  if (parentChunks.length === 0) {
    console.error("❌ アップロード対象のチャンクが見つかりません");
    process.exit(1);
  }

  console.log(`📚 ${parentChunks.length}件の親チャンクを読み込みました`);

  if (shouldClear) {
    await clearExistingRagData();
  }

  for (const [index, chunk] of parentChunks.entries()) {
    console.log(`➡️  [${index + 1}/${parentChunks.length}] ${chunk.id} - ${chunk.title}`);
  ...
  console.log("✅ ナナのRAGデータをSupabaseに登録しました");
}
